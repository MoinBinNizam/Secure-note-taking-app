...
const Sidebar = ({ isOpen, toggle }) => (
  <aside className={`${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 fixed lg:static z-50 w-[260px] h-screen bg-white border-r border-slate-200 flex flex-col transition-transform duration-300`}>
    ...
    <div className="p-6 border-t border-slate-100">
      <button onClick={() => { localStorage.removeItem('token'); window.location.href = '/login'; }} className="w-full flex items-center gap-3 text-slate-500 font-semibold hover:text-red-600 transition-all px-4">
