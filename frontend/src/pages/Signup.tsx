        ...
        <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
            <div className="w-full max-w-md bg-emerald-50 p-8 rounded-3xl shadow-2xl border border-emerald-100">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div>
                        <label className="block text-sm font-semibold text-emerald-800 mb-2">Interests</label>
                        <input
                            type="text"
                            className="w-full px-4 py-3 border border-emerald-200 bg-white rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all"
                            value={interest}
                        ...
                        <div className="flex flex-wrap gap-2 mt-3">
                            {interests.map((tag) => (
                                <span key={tag} className="flex items-center px-3 py-1 bg-emerald-200 text-emerald-800 text-xs font-semibold rounded-full">
                                    {tag}
                                    <button type="button" onClick={() => removeInterest(tag)} className="ml-2 hover:text-emerald-950">&times;</button>
                                </span>
                            ))}
                        </div>
                    </div>
