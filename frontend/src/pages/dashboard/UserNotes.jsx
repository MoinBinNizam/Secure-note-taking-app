...
const UserNotes = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 pb-20">
      ...
      <NoteWindow 
        isOpen={modal.isOpen} 
        onClose={() => setModal({ isOpen: false, note: null })} 
        title={modal.note ? 'Edit Note' : 'Create New Note'}
      >
        <div className="space-y-4">
            <input required className="w-full px-4 py-3 bg-white border border-slate-200 rounded-2xl" placeholder="Note Title" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} />
            
            <div className="border border-slate-200 rounded-2xl overflow-hidden min-h-[300px] flex flex-col bg-white">
                ...
                <div 
                    ref={contentRef} 
                    contentEditable 
                    className="w-full px-4 py-4 bg-white flex-grow h-[300px] outline-none overflow-y-auto cursor-text"
                />
            </div>
